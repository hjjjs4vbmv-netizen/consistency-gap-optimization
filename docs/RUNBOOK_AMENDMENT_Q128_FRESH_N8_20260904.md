...
## Owner-directed operational waiver 003

At `2026-09-03T05:04:03Z`, before any formal training log and before observation of any formal quality value, the experiment owner explicitly directed that no further GPU time be spent on seed999 smoke and that formal seeds 201–208 start immediately. The unpassed seed999 smoke and exact-resume/switch parity gates are therefore recorded as `WAIVED_NOT_PASSED`; they are not represented as PASS. Preflight, asset hashes, GPU identity, source freeze, seed-to-GPU binding, deterministic arm order, immutable checkpoints, opaque evaluation, and all scientific/statistical definitions remain mandatory.

This waiver is an operational protocol deviation and increases the risk that a later resume/switch defect invalidates crossed-history outputs. It does not change the cohort, arms, training command, hypotheses, metrics, evaluation matrix, missingness rules, or success thresholds, and it is not informed by FID/KID or other quality outcomes. The machine-readable record is `analysis/q128_fresh_regime_history_n8_v1/owner_directed_operational_waiver_003.json`. The launch receipt must expose the waiver and the two unpassed gates. A new pushed freeze commit must still precede every formal log.
