...
\subsection{实测诊断证据(2026-08-10,已执行)}
诊断定位已由实测数据支撑(详见 `analysis/e5\_residual\_structure\_result.md`、
`analysis/e6\_dose\_response\_result.md`):

\begin{itemize}
  \item \textbf{E5 残差有结构(非噪声)。} 坐标 gauge 偏离 $1$ 的程度随层深增大
        (pearson $+0.46$);残差与更新幅值无关;梯度方向残差随 $|g-1|$ 单调增长。
        层级 gauge 偏离小($\sim3$--$5\%$)且集中,无单一"坏层"。
  \item \textbf{E6 剂量响应:残差与 FID 强反相关。} 残差单调于 $|g-1|$;而
        FID 在 $g{=}1.0$(零残差)最差、$g{=}1.3$(最大残差)最佳
        ($\mathrm{corr}(\text{残差},\text{FID})\approx -0.99$)。零残差臂最差,
        残差无害。
  \item \textbf{E7 优化器状态依赖(已用直接法完成)。} 绕过审计的 sigmoid 门控,
        直接调用 \texttt{run\_stateful\_pair} 在 arm\_a 各 tick(不同 $n_K$)上测量:
        梯度残差 $R_{\mathrm{grad}}$ 随优化器成熟度增长($n_K{=}243\to1991$,
        $R_{\mathrm{grad}}{=}0.031\to0.091$),残差是\emph{状态条件}的。更新失真
        $R_{\mathrm{opt}}$ 小且平坦($0.012$--$0.017$),$R_{\mathrm{opt}}-R_{\mathrm{grad}}$
        为负(优化器压缩残差)。注意:残差大小与 $R_{\mathrm{opt}}-R_{\mathrm{grad}}$
        符号对测量口径敏感(此前审计 $R_{\mathrm{opt}}{=}0.086$、$+0.027$;此处
        $0.014$、$-0.076$),需报告口径。
  \item \textbf{E11 稳健性(已用现有跨 seed 数据完成)。} 1024k 预算下较大 gap 臂
        (g=1.1)在全部 3 个 seed 上赢 FID-50k(5/6 cell);moment-memory 的
        $\mathrm{Corr}\approx 0$ 在 atol 1e-5/1e-6 下稳定。
  \item \textbf{E3 普适性(已用 AdamW 完成)。} 梯度残差 $R_{\mathrm{grad}}$ 跨
        optimizer 复现(AdamW $0.095$ vs.\ RAdam $0.091$),是非标量梯度内容为
        loss/梯度属性的证据,非 RAdam 特有;更新失真 $R_{\mathrm{opt}}$ 随
        optimizer 不同(AdamW 放大 $0.39$,RAdam 压缩 $0.014$)。
\end{itemize}

六条实测结论(E5 结构、E6 剂量、E7 状态条件、E11 稳健、E3 普遍、g=1.3-vs-1.0
FID)共同钉死诊断定位:非标量残差存在、有结构、状态条件、普遍、无害——带它的
臂质量更好。
...
干预被反指。四重实测证据(Arm C scale-matched control、P1 附庸性检验、梯度级未来预测、
优化器级未来预测)一致指向:非标量梯度/优化器残差是真实但无后果的现象——质量改善
大部分是平凡 LR 重缩放(Arm C 消除 84--100\%)、残差附庸于标量 gap(偏相关
$\approx 0$)、早期 $R_{\mathrm{grad}}$ 与早期 $R_{\mathrm{opt}}$ 都不预测未来 FID
(两者跨 seed 均近恒定)。优化器记忆机制仍真实($h_{\mathrm{actual}}\neq h_{\mathrm{pred}}$,
$R_{\mathrm{opt}}$ 对 gap 敏感),但其质量后果大部分平凡,且机制前 3 环成立而最后
一环 Performance 在梯度级与优化器级均不成立。论文定位为诚实的诊断论文(非标量
梯度内容的存在、结构与质量方向,约 $20$--$30\%$),可辩护但非"化腐朽为神奇"。
Go/No-Go 定论:No-Go for ICLR 主会机制论题。目标自始至终唯一:使论文进入
ICLR 2027——但须基于对前提的实测检验,而非乐观假设。
