...
\begin{itemize}
  ...
  \item \textbf{E6 剂量响应:残差与 FID 强反相关。} 残差单调于 $|g-1|$;而
        ...
        残差无害。
  \item \textbf{E7 优化器状态依赖:基础设施受阻。} 审计要求 checkpoint 与
        training-state 均为 sigmoid,而 arm 状态为 global\_sigmoid、g\_screen
        g1\_0 仅一个状态——无法在不动核心审计的前提下测多状态。列为待办/受阻。
\end{itemize}

三条实测结论(E5 结构、E6 剂量、g=1.3-vs-1.0 FID)共同钉死诊断定位:非标量残差
存在、有结构、无害——带它的臂质量更好。
