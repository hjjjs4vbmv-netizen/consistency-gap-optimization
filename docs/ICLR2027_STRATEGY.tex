...
干预被反指。三重实测证据(Arm C scale-matched control、P1 附庸性检验、未来预测
梯度级阴性)一致指向:非标量梯度残差是真实但(梯度级)无后果的现象——质量改善
大部分是平凡 LR 重缩放(Arm C 消除 84--100\%)、残差附庸于标量 gap(偏相关
$\approx 0$)、早期梯度残差不预测未来 FID。优化器记忆机制仍真实
($h_{\mathrm{actual}}\neq h_{\mathrm{pred}}$),但其质量后果大部分平凡。唯一剩余活口是
优化器级未来预测(早期 $R_{\mathrm{opt}}$,需重训存早期 optimizer state),但鉴于
Arm C 翻盘概率不高。论文定位为诚实的诊断论文(非标量梯度内容的存在、结构与质量
方向,约 $20$--$30\%$),可辩护但非"化腐朽为神奇"。E2 降级为探索性消融与设计启示。
Go/No-Go 交 leader 判断:是否花计算测优化器级未来预测,还是接受三重证据定 workshop。
目标自始至终唯一:使论文进入 ICLR 2027——但须基于对前提的实测检验,而非乐观假设。
