...
\begin{abstract}
Consistency training pairs states separated along a noise trajectory, yet the
optimization meaning of changing their realized spacing is unclear. We study
a controlled ECT pair-spacing perturbation and show that it is a composite
intervention that exactly decomposes into a detached target-endpoint change and
an explicit loss-rescaling change. These components
...
We treat the gap as the realized pair spacing $\Delta=t-r$, not as a new
mechanism independent of the schedule that generated it. In the present
experiments, $q=256$ fixes the baseline ECT mapping and $g=1.10$ is a
controlled probe that scales its realized spacing. We have not established an
effect of $g$ that is independent of $q$; the estimand is conditional on this
baseline schedule. In the ECT objective studied here, perturbing $\Delta$
jointly changes the detached target endpoint and the explicit loss weight.
These changes are exactly separable at one fixed model state, but separate
training runs feed their gradients into different parameter and optimizer
trajectories. Objective-level identities therefore constrain the instantaneous
intervention without determining finite-training outcomes.
...
Consistency models were originally formulated by discretizing a probability
flow ODE and matching predictions at neighboring noise levels, with the
discretization schedule and loss weight left as explicit design choices
\citep{song2023consistency}. Improved consistency training subsequently changed
both the discretization curriculum and the weighting rule, including the
inverse-spacing weight $1/(t_{i+1}-t_i)$
\citep{song2024improved}. ECT replaces non-overlapping intervals with a
continuous pair mapping $p(r\mid t,\mathrm{iters})$, uses $q$ to shrink the
spacing during training, and explicitly observes that $1/(t-r)$ couples the
weighting rule to that mapping \citep{geng2024consistency}. Pair spacing,
curriculum, and weighting are therefore established parts of consistency-model
design rather than novelty claims of this paper.

Adjacent work removes discrete schedules through continuous-time objectives
\citep{lu2024simplifying}, analyzes consistency-model discretization complexity
\citep{yang2025discretization}, learns adaptive discretizations together with
an adaptive weight \citep{bai2025adaptive}, and develops broader flow-map and
self-distillation formulations \citep{boffi2025how}. Our contribution is
narrower: for the one-sided stop-gradient ECT objective, we derive an exact
finite-spacing target-endpoint plus explicit-rescaling decomposition, construct
matched objective-gradient factorial identities, and test what remains after
the corresponding arms enter separate finite-training trajectories.
...
\end{equation}
Thus $q$ and $g$ are not two proven-independent mechanisms: both parameterize
the realized pair. Conditional on a fixed stage and away from clipping, a
...
\claimnote{Do not describe $q$ and $g$ as causally independent factors. The
current result is conditional on $q=256$ and identifies a controlled realized
pair-spacing intervention.}
...
This fixes the baseline $q$ schedule and probes its realized pair spacing; it
is not a $q\times g$ factorial and does not establish a $q$-independent effect.
...
\paragraph{What the $q$-fixed design identifies.}
The present factorial identifies the effect of a controlled spacing
perturbation conditional on the $q=256$ curriculum. It does not compare
alternative $q$ curricula. A future parameterization control would hold the
realized pairs $(t,r)$ pointwise fixed while generating them through a
stage-specific $q$ rule rather than the global $g$ wrapper. Such a control is
an implementation-equivalence test: once $(t,r)$ and the explicit weight are
matched, the ECT objectives should also match. This control is specified for
future work and is not part of the current evidence.
...
A controlled pair-spacing change in ECT is an exact composite intervention: it
changes both the detached target endpoint and the explicit loss denominator.
Those components factorize at the instantaneous objective-gradient level, but the
