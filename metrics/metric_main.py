...
def calc_metric(metric, **kwargs): # See metric_utils.MetricOptions for the full list of arguments.
    ...
    opts = metric_utils.MetricOptions(metric_name=metric, **kwargs)
...
@register_metric
def fid512_full(opts):
    """512 generated-sample FID metric for pipeline smoke testing."""
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=512)
    return dict(fid512_full=fid)

@register_metric
def kid512_full(opts):
    """512 generated-sample KID metric for pipeline smoke testing."""
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    kid = kernel_inception_distance.compute_kid(
        opts,
        max_real=None,
