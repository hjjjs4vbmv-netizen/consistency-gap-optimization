#!/usr/bin/env bash
set -Eeuo pipefail
repo="${1:?usage: run_formal_pipeline.sh REPO PROTOCOL DATASET TRANSFER RUNTIME_SIF CACHE_ROOT KID_REAL_FEATURES FID_REAL_FEATURES FEATURE_DETECTOR EXPERIMENT_ROOT}"
...
feature_detector="${9:?missing feature detector}"; root="${10:?missing experiment root}"
...
apptainer exec --bind /data:/data --pwd "${repo}" "${runtime_sif}" python \
  analysis/q256_p2_b384_pulse_chase_v1/prepare_evaluation_manifest.py \
    ...
    --fid-real-features "${fid_real}" --feature-detector "${feature_detector}" \
    --output "${root}/evaluation_manifest.json"
