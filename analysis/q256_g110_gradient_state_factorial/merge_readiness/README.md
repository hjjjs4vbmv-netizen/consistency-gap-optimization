...
standalone-script import and adds a regression assertion. The order-dependent
global `sys.path[0]` assertion is replaced with an isolated subprocess check of
the checker's actual contract: when the repository root is absent, importing
the checker inserts it first. The comparison logs are in
