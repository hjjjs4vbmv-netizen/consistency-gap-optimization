...
worker=${bundle}/run_learning_curve_replay_recovery_v2_worker.sh
...
for required in \
  ...
  "${worker}" "${inventory_builder}" "${trajectory_verifier}" "${milestone_adopter}" \
...
for required in \
  ...
  "${original_1024_root}" "${failed_replay_root}" "${evaluator_source_root}" "${cache_template}"; do
...
for seed in 14 15 16 17 18; do
  failed_run=${failed_replay_root}/seed${seed}/armA
  [[ -s "${failed_run}/training-state-latest.pt" && -s "${failed_run}/network-snapshot-latest.pkl" ]] || { echo "missing failed-v1 384 durability checkpoint: seed${seed}" >&2; exit 2; }
  grep -q "AttributeError: module 'torch_utils.distributed' has no attribute 'barrier'" "${failed_replay_root}/seed${seed}-worker.log" || { echo "unexpected failed-v1 cause: seed${seed}" >&2; exit 2; }
  last=$(tail -n 1 "${failed_run}/train_summary.csv")
  [[ "$(printf '%s\n' "${last}" | awk -F, '{print $1}')" == 3000 && "$(printf '%s\n' "${last}" | awk -F, '{print $4}')" == 384.000000 ]] || { echo "failed-v1 did not stop at exact 384 kimg: seed${seed}" >&2; exit 2; }
  [[ ! -e "/tmp/ECT001-q256-learning-curve-replay-recovery-v2-shm-seed${seed}" ]] || { echo "existing recovery replay shm for seed ${seed}" >&2; exit 3; }
  tmux has-session -t "q256lcr2_s${seed}" 2>/dev/null && { echo "existing tmux session q256lcr2_s${seed}" >&2; exit 3; } || true
done
...
cp "${worker}" "${inventory_builder}" "${trajectory_verifier}" "${milestone_adopter}" \
...
cp "${failed_replay_root}/replay_source_inventory.json" "${inventory}"
[[ "$(sha256sum "${failed_replay_root}/replay_source_inventory.json" | awk '{print $1}')" == "$(sha256sum "${inventory}" | awk '{print $1}')" ]] || { echo "copied inventory hash mismatch" >&2; exit 4; }
...
{
  "schema": "ect.q256.learning-curve-replay-recovery-launch/v2",
  "status": "LAUNCHED",
  "launch_utc": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "source_commit": "dcca41b19e7c45512b5fbe98776520396a1bf9ac",
  "replay_commit": "${replay_commit}",
  "recovery_from": "${failed_replay_root}",
  "recovery_reason": "output-only milestone saver called unavailable dist.barrier after exact 384 durability checkpoint",
  "armA_384_policy": "hash-identical adoption into new root; no retraining from 256",
...
for row in '0 14 31114' '1 15 31115' '2 16 31116' '3 17 31117' '4 18 31118'; do
  ...
  tmux new-session -d -s "q256lcr2_s${seed}" \
    "bash '${worker}' '${gpu_index}' '${seed}' '${master_port}' '${replay_commit}' > '${run_root}/seed${seed}-worker.log' 2>&1"
  tmux display-message -p -t "q256lcr2_s${seed}" '#{session_name} #{pane_pid}' >"${run_root}/seed${seed}-tmux.txt"
done

sleep 5
for seed in 14 15 16 17 18; do
  tmux has-session -t "q256lcr2_s${seed}" 2>/dev/null || { echo "recovery replay session died during startup: seed${seed}" >&2; exit 5; }
done

echo "REPLAY_RECOVERY_V2_LAUNCH_PASS utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) replay_commit=${replay_commit} inventory=${inventory_status}:${inventory_pass}/20 adopted_armA_384=5 checkpoints=120 evaluation_jobs=240"
