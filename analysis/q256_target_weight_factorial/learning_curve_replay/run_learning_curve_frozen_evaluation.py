#!/usr/bin/env python3
"""Run one seed's frozen q256 intermediate learning-curve evaluation."""
...
PROTOCOL = "q256-target-weight-learning-curve-formal-evaluation-v1"
...
def validate_training_cell(
    training_root: Path, seed: int, arm: str, budget_kimg: int
) -> dict[str, Any]:
    ...
    milestone_dir = run_dir / f"kimg{budget_kimg:04d}"
    checkpoint = milestone_dir / "network-snapshot.pkl"
    milestone_receipt = milestone_dir / "milestone_receipt.json"
    checkpoint_manifest = run_dir / "checkpoint_manifest.json"
    completion = run_dir / "trajectory_completion_receipt.json"
    ...
    receipt = json.loads(milestone_receipt.read_text(encoding="utf-8"))
    ...
    if (
        ...
        or receipt.get("milestone_kimg") != budget_kimg
    ):
        raise EvaluationFailure(f"milestone binding mismatch: {milestone_receipt}")
    if (
        ...
        raise EvaluationFailure(f"trajectory manifest/completion mismatch: {run_dir}")
    manifest_item = next(
        (item for item in manifest.get("milestones", []) if item.get("kimg") == budget_kimg),
        None,
    )
    checkpoint_sha = sha256_file(checkpoint)
    if (
        manifest_item is None
        or manifest_item.get("evaluation_snapshot", {}).get("sha256")
        != checkpoint_sha
    ):
        raise EvaluationFailure(f"checkpoint manifest mismatch: {checkpoint}")
    ...
    return {
        ...
        "milestone_receipt": str(milestone_receipt),
        "milestone_receipt_sha256": sha256_file(milestone_receipt),
...
def build_jobs(
    *, source_root: Path, output_root: Path, dataset: Path,
    ...
    for nfe, mid_t in NFE_SETTINGS.items():
        for cell in cells:
            job_id = (
                f"seed{cell['seed']}-arm{cell['arm']}-"
                f"kimg{cell['budget_kimg']:04d}-nfe{nfe}"
            )
    ...
    if len(jobs) != 48:
        raise EvaluationFailure(f"one-seed matrix must contain 48 jobs, got {len(jobs)}")
    return jobs
...
def main() -> int:
    ...
    try:
        ...
        cells = [
            validate_training_cell(training_root, args.seed, arm, budget)
            for arm in ARMS
        ...
        plan = {
            "schema": "q256-target-weight-seed14-18-learning-curve-evaluation-plan-v1",
            "status": "RUNNING",
            "created_at_utc": utc_now(),
            "training_source_commit": args.replay_commit,
            "parent_256k_training_source_commit": PARENT_TRAINING_COMMIT,
            "training_budget_kimg": list(BUDGETS),
            ...
            "protocol": {
                ...
                "selection_policy": "all_replay_milestones_NFE1_first_then_NFE2_no_selection",
            },
            "runtime": runtime,
        ...
        for job in jobs:
            ...
            receipt = {
                "schema": "q256-target-weight-seed14-18-learning-curve-evaluation-job-receipt-v1",
        ...
        completion = {
            "schema": "q256-target-weight-seed14-18-learning-curve-evaluation-worker-completion-v1",
        ...
        failure = {
            "schema": "q256-target-weight-seed14-18-learning-curve-evaluation-failure-v1",
