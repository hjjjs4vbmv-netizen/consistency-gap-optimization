...
repo=/data/temp/ECT001/q256-learning-curve-replay-source-recovery-v2
...
failed_replay_root=/data/raw/ECT/ect_runs/q256-target-weight-factorial-20260821/secondary-precision-extension/seed14-18-256to1024-learning-curve-replay-v1
run_root=/data/raw/ECT/ect_runs/q256-target-weight-factorial-20260821/secondary-precision-extension/seed14-18-256to1024-learning-curve-replay-recovery-v2
...
bundle=/data/temp/ECT001/q256-learning-curve-replay-bundle-recovery-v2
...
milestone_adopter=${bundle}/adopt_failed_384_milestone.py
inventory=${run_root}/replay_source_inventory.json
private_shm="/tmp/ECT001-q256-learning-curve-replay-recovery-v2-shm-seed${seed}"
...
write_failure() {
  ...
  if [[ -d "${run_root}" && ! -e "${failure_path}" ]]; then
    {
      printf '# q256 seed14-18 learning-curve replay recovery-v2 failure\n\n'
...
[[ -d "${source_training_root}" && -d "${original_1024_root}" && -d "${failed_replay_root}" && -d "${evaluation_root}" && -d "${evaluator_source_root}" && -d "${cache_template}" ]] || { echo "missing training/evaluation root" >&2; exit 2; }
[[ -f "${dataset}" && -f "${canonical_dataset}" && -f "${evaluator_source_archive}" && -f "${evaluation_worker}" && -f "${evaluation_driver}" && -f "${trajectory_verifier}" && -f "${milestone_adopter}" && -f "${inventory}" ]] || { echo "missing immutable asset, inventory, recovery tool, verifier, or evaluator" >&2; exit 2; }
...
run_arm() {
  ...
  local formal_source_state=${source_dir}/training-state-latest.pt
  local resume_state=${formal_source_state}
  local original_1024_state=${original_1024_root}/seed${seed}/arm${arm}/training-state-latest.pt
  active_arm=${arm}
  [[ ! -e "${outdir}" ]] || { echo "refusing existing recovery cell: ${outdir}" >&2; return 3; }
  mkdir -p "${run_root}/seed${seed}"
  [[ -f "${formal_source_state}" && -f "${original_1024_state}" ]] || { echo "missing 256k source or original 1024 state: seed${seed}/arm${arm}" >&2; return 3; }
  ...
  if [[ "${arm}" == A ]]; then
    ...
    proot -0 -r "${runtime}" \
      ...
      /usr/bin/env -i HOME=/root LC_ALL=C.UTF-8 PYTHONNOUSERSITE=1 PYTHONPATH="${repo}" \
        ...
        /usr/bin/python "${milestone_adopter}" \
          --failed-run-dir "${failed_replay_root}/seed${seed}/armA" \
          --recovery-run-dir "${outdir}" \
          --clean-source-log "${source_dir}/log.txt" \
          --failure-log "${failed_replay_root}/seed${seed}-worker.log" \
          --seed "${seed}"
    resume_state=${outdir}/training-state-latest.pt
    active_arm=A
  else
    mkdir "${outdir}"
  ...
  python3 -c "import json,os; p='${outdir}/run_config.json'; payload={'schema':'ect.q256.learning-curve-run-config/v1','seed':${seed},'arm':'${arm}','gpu_index':${gpu_index},'formal_source_state':'${formal_source_state}','resume_state':'${resume_state}','source_state_sha256':next(x for x in json.load(open('${inventory}'))['cells'] if x['seed']==${seed} and x['arm']=='${arm}')['files']['training-state-latest.pt']['sha256'],'source_commit':'dcca41b19e7c45512b5fbe98776520396a1bf9ac','replay_commit':'${replay_commit}','target_kimg':1024,'milestones_kimg':[384,512,640,768,896,1024],'target_gap_scale':${target_scale},'denominator_gap_scale':${denominator_scale},'recovery_from_barrier_failure':${arm@Q}=='A'}; f=open(p,'x'); json.dump(payload,f,indent=2,sort_keys=True); f.write('\\n'); f.flush(); os.fsync(f.fileno())"
  echo "[worker] ARM_START utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) seed=${seed} arm=${arm} target=${target_scale} denominator=${denominator_scale} mode=strict_budget_only_resume resume=${resume_state} target_kimg=1024"
  ...
  proot -0 -r "${runtime}" \
    ...
    /usr/bin/env -i HOME=/root LC_ALL=C.UTF-8 PYTHONNOUSERSITE=1 PYTHONPATH="${repo}" \
      ...
      /usr/bin/python "${trajectory_verifier}" \
        --run-dir "${outdir}" \
        --source-state "${formal_source_state}" \
