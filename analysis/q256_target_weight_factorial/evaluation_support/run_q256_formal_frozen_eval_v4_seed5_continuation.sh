...
repo=/data/temp/ECT001/q256-factorial-eval-feature-reuse-v4
...
outdir=${evaluation_parent}/frozen-eval-feature-reuse-primary-first-v4-seed5-continuation
log=${evaluation_parent}/formal-frozen-eval-feature-reuse-primary-first-v4-seed5-continuation.log
adapter=/data/temp/ECT001/run_q256_direct_frozen_evaluation_v4.py
...
[[ -d "${matrix_dir}" && -f "${prior_root}/evaluation_completion.json" ]] || { echo 'missing matrix or prior fail-closed receipt' >&2; exit 2; }
[[ -d "${private_shm}" && ! -L "${private_shm}" && "$(stat -c '%U:%a' "${private_shm}")" == 'ECT001:700' ]] || { echo 'invalid private shared-memory directory' >&2; exit 2; }
[[ ! -e "${outdir}" ]] || { echo 'refusing an existing v4 continuation root' >&2; exit 3; }
...
exec >>"${log}" 2>&1
echo "[formal-evaluation-v4] START utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) gpu=${gpu_uuid} evaluator_head=$(cd "${repo}" && git rev-parse HEAD) training_head=${training_head} continuation=seed5-armA-nfe1"
...
echo "[formal-evaluation-v4] PASS utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
