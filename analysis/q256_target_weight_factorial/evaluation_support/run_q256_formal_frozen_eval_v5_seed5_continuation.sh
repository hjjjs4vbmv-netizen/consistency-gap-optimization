...
repo=/data/temp/ECT001/q256-factorial-eval-feature-reuse-v5
...
outdir=${evaluation_parent}/frozen-eval-feature-reuse-primary-first-v5-seed5-continuation
log=${evaluation_parent}/formal-frozen-eval-feature-reuse-primary-first-v5-seed5-continuation.log
adapter=/data/temp/ECT001/run_q256_direct_frozen_evaluation_v5.py
...
[[ -d "${matrix_dir}" && -f "${prior_root}/evaluation_completion.json" ]] || { echo 'missing matrix or prior prefix receipt' >&2; exit 2; }
...
[[ ! -e "${outdir}" ]] || { echo 'refusing an existing v5 continuation root' >&2; exit 3; }
...
exec >>"${log}" 2>&1
echo "[formal-evaluation-v5] START utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) gpu=${gpu_uuid} evaluator_head=$(cd "${repo}" && git rev-parse HEAD) training_head=${training_head} continuation=seed5-armA-nfe1 audit_backend=pynvml+procfs"
...
echo "[formal-evaluation-v5] PASS utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
