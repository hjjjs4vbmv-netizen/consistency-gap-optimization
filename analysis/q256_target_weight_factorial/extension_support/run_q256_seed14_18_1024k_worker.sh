...
evaluation_worker=/data/temp/ECT001/q256-seed14-18-1024k-v1/run_q256_seed14_18_1024k_native_evaluation_worker.sh
evaluation_driver=/data/temp/ECT001/q256-seed14-18-1024k-v1/run_q256_seed14_18_1024k_frozen_evaluation.py
private_shm="/tmp/ECT001-q256-seed14-18-1024k-v1-shm-seed${seed}"
...
write_failure() {
  ...
  if [[ -d "${run_root}" && ! -e "${failure_path}" ]]; then
    {
      printf '# q256 seed14-18 1024k pipeline failure\n\n'
...
[[ -d "${source_training_root}" && -d "${evaluation_root}" && -d "${evaluator_source_root}" && -d "${cache_template}" ]] || { echo "missing training/evaluation root" >&2; exit 2; }
