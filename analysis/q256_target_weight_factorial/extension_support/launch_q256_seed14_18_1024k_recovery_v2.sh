...
bundle=/data/temp/ECT001/q256-seed14-18-1024k-recovery-v2
source_bundle=/data/temp/ECT001/q256-seed14-18-1024k-v1
source_archive=${source_bundle}/q256-training-source-4582051.tar
source_archive_sha_file=${source_bundle}/q256-training-source-4582051.tar.sha256
repo=/data/temp/ECT001/q256-factorial-1024k-4582051-seed14-18-v1
worker=${bundle}/run_q256_seed14_18_1024k_recovery_v2_worker.sh
...
(cd "${source_bundle}" && sha256sum -c "$(basename "${source_archive_sha_file}")")
...
[[ -d "${source_training_root}" && -d "${failed_root}" && -d "${repo}" && -d "${evaluator_source_root}" && -d "${cache_template}" && -f "${evaluator_source_archive}" ]] || { echo "missing source training/evaluator asset" >&2; exit 2; }
...
for seed in 14 15 16 17 18; do
  ...
  [[ -f "${failed_root}/seed${seed}-worker-failure.md" ]] || { echo "failed-v1 receipt missing for seed${seed}" >&2; exit 2; }
  grep -q "ModuleNotFoundError: No module named 'torch_utils'" "${failed_root}/seed${seed}-worker.log" || { echo "unexpected failed-v1 cause for seed${seed}" >&2; exit 2; }
  failed_a=${failed_root}/seed${seed}/armA
  for required in training-state-latest.pt network-snapshot-latest.pkl training_options.json train_summary.csv factorial_training_telemetry_v1.csv initial_state_receipt_v1.json log.txt; do
    [[ -s "${failed_a}/${required}" ]] || { echo "missing failed-v1 armA artifact: ${failed_a}/${required}" >&2; exit 2; }
  done
  last=$(tail -n 1 "${failed_a}/train_summary.csv")
  [[ "$(printf '%s\n' "${last}" | awk -F, '{print $1}')" == 8000 && "$(printf '%s\n' "${last}" | awk -F, '{print $4}')" == 1024.000000 && "$(printf '%s\n' "${last}" | awk -F, '{print $7}')" == 0 ]] || { echo "invalid failed-v1 armA endpoint: seed${seed}: ${last}" >&2; exit 2; }
  for arm in B C D; do
...
for seed in 14 15 16 17 18; do
  [[ ! -e "/tmp/ECT001-q256-seed14-18-1024k-recovery-v2-shm-seed${seed}" ]] || { echo "existing recovery-v2 private shm for seed ${seed}" >&2; exit 3; }
  tmux has-session -t "q1024r2_s${seed}" 2>/dev/null && { echo "existing tmux session q1024r2_s${seed}" >&2; exit 3; } || true
done
...
cp "$0" "${run_root}/provenance/"
{
  ...
  printf 'recovery_from=%s\n' "${failed_root}"
  printf 'recovery_reason=post_training_state_verifier_missing_PYTHONPATH_torch_utils\n'
  printf 'completed_armA_policy=hash_identical_copy_into_new_recovery_root_no_retraining\n'
  printf 'remaining_arms=B,C,D\n'
...
for seed in 14 15 16 17 18; do
  sha256sum \
    "${failed_root}/seed${seed}/armA/training-state-latest.pt" \
    "${failed_root}/seed${seed}/armA/network-snapshot-latest.pkl" \
    "${failed_root}/seed${seed}/armA/train_summary.csv"
done >"${run_root}/provenance/failed_v1_completed_armA_artifacts.sha256"

for row in '0 14 30114' '1 15 30115' '2 16 30116' '3 17 30117' '4 18 30118'; do
  read -r gpu_index seed master_port <<<"${row}"
  tmux new-session -d -s "q1024r2_s${seed}" \
    "bash '${worker}' '${gpu_index}' '${seed}' '${master_port}' > '${run_root}/seed${seed}-worker.log' 2>&1"
  tmux display-message -p -t "q1024r2_s${seed}" '#{session_name} #{pane_pid}' >"${run_root}/seed${seed}-tmux.txt"
done

sleep 5
for seed in 14 15 16 17 18; do
  tmux has-session -t "q1024r2_s${seed}" 2>/dev/null || { echo "recovery-v2 session died during startup: seed${seed}" >&2; exit 4; }
done

echo "RECOVERY_V2_LAUNCH_PASS utc=$(date -u +%Y-%m-%dT%H:%M:%SZ) seeds=14,15,16,17,18 adopted_armA=true remaining_arms=B,C,D one_gpu_per_seed=true target_kimg=1024 evaluation_chained=true"
