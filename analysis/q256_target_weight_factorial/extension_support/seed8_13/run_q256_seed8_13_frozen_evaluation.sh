...
adapter=/data/temp/ECT001/q256-seed8-13-v1/run_q256_seed8_13_frozen_evaluation.py
formal_adapter=/data/temp/ECT001/run_q256_direct_frozen_evaluation_v6.py
sandbox=/data/temp/ect001-pytorch2401-sandbox
private_shm=/tmp/ECT001-q256-seed8-13-extension-shm-gpu0
...
[[ "${gpu_processes}" == 0 ]] || { echo 'evaluation GPU is not compute-idle' >&2; exit 2; }
if ss -H -ltn | awk '$4 ~ /:339[0-4][0-9]$/ {found=1} END {exit !found}'; then
  echo 'one or more frozen evaluation ports 33900-33949 are busy' >&2
  exit 2
fi
