#!/usr/bin/env python3
"""Bind seed8-13 extension outputs and run the frozen q256 evaluator.

This adapter changes only the matrix membership (seeds 8 through 13), provenance
...
def create_binding(
    ...
    report_path = extension_root / "q256_factorial_seed8_13_extension_report.json"
...
def load_bound_matrix(
    ...
    if not isinstance(audits, list) or len(audits) != 6:
        fail("matrix binding lacks six audit receipts")
    ...
    if not isinstance(receipts, list) or len(receipts) != 24:
        fail("extension matrix binding must contain 24 cells")
...
def build_jobs(
    ...
    if len(jobs) != 48:
        fail(f"extension evaluation must contain exactly 48 jobs, got {len(jobs)}")
    return jobs
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def build_plan(**kwargs: Any) -> dict[str, Any]:
        ...
        plan["selection_policy"] = "all_24_extension_final_256kimg_checkpoints"
    ...
    def write_json(path: Path, value: Mapping[str, Any]) -> None:
        ...
        if path.name == "evaluation_completion.json":
            payload["selection_policy"] = (
                "all_24_extension_final_256kimg_checkpoints"
            )
            payload["independent_training_seed_n"] = 6
