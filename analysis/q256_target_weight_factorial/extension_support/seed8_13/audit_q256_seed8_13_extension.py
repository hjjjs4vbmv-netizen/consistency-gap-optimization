...
def audit_seed(root: Path, seed: int, *, check_only: bool) -> dict[str, Any]:
    if seed not in (8, 9, 10, 11, 12, 13):
        fail("extension audit seed must be in 8 through 13")
    root = root.resolve()
...
def generate_extension_report(root: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "seeds": {str(seed): audits[seed] for seed in (8, 9, 10, 11, 12, 13)},
    ...
    json_path = root / "q256_factorial_seed8_13_extension_report.json"
    md_path = root / "q256_factorial_seed8_13_extension_report.md"
...
def main() -> int:
    ...
    audit.add_argument("--seed", type=int, choices=(8, 9, 10, 11, 12, 13), required=True)
