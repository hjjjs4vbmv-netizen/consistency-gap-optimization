...
**状态：** 原正式实验已完成：52/64 训练成功，12/64 科学数值失败；260 个有效评估和 60 个无终点槽均已核算。主结果 INCONCLUSIVE（7/16 完整 R 配对）；见 [完整结果](results/README.md)。
...
## 13. 完成记录与独立 FP32 敏感性附件

本节于 2026-09-07 整理发布，记录已获用户授权的执行及失败后补充工作；
不是在结果产生前新登记的确认性方案，不修改第 1–12 节的原主要估计对象。
原 roster 为 seeds 50–65。原 64 分支仍保留 52 PASS / 12 SCIENTIFIC_FAILURE，
对应 260 个有效评估和 60 个 NO_ENDPOINT 槽；后者在分析中属于科学缺失，不能插补。
原正式主集合为 55、56、59、60、61、62、65；四臂集合去掉 65。

失败后的原参数续跑和定点数值诊断不替代原科学失败。随后于 2026-09-06 分批获授权，
对全部 12 条原失败分支执行独立 FP32 敏感性续跑：只关闭网络内部 FP16，
保留恢复点的 optimizer、GradScaler、EMA、RNG/sampler、LR、batch 和其他学习状态，
不重新执行 512 时的 optimizer reset 或 EMA 初始化。目标仍为 attempt8000 / 1024 kimg。
科学归属统一为 `POST_FAILURE_FP32_SENSITIVITY`，不填回原正式队列，不启动 M2 或新 seed。

| seed | 分支 | 恢复 attempt | 起点类型 |
|---|---|---:|---|
| 50 | R_A | 6321 | 最近保存有限状态 |
| 51 | R_A | 6085 | 精确失败前重放状态 |
| 52 | R_A | 6321 | 最近保存有限状态 |
| 53 | R_A | 7901 | 最近保存有限状态 |
| 54 | R_A | 7111 | 最近保存有限状态 |
| 57 | R_A | 6321 | 最近保存有限状态 |
| 58 | K_A | 7901 | 最近保存有限状态 |
| 58 | K_B | 5531 | 最近保存有限状态 |
| 58 | R_B | 7111 | 最近保存有限状态 |
| 63 | R_A | 5531 | 最近保存有限状态 |
| 64 | R_A | 4008 | 精确失败前重放状态 |
| 65 | K_A | 4000 | 最近保存有限状态 |

这些精度干预起点不同，不构成从共同 512 起点开始的全 FP32 四臂实验。
12 条均到达终点；各复用原 E_512 B0/B1/B2、E_KEEP B0、ONLINE B0 五个读出，
共 60 项独立评估。evaluator commit、FP32 评估、NFE1、FID50k/KID50k、样本 IDs、
metric RNG 和 batch 均保持原评估配置。结果只作失败后敏感性描述。

公开附件提供全部 380 行状态/指标、逐 seed 统计、成败表和离线复算脚本；
未附 checkpoint、生成图像、特征数组及服务器运维记录。
该附件可复算质量统计，不等于发布完整训练重放环境或重新审计 CRN/runner 实现。
