...
protocol="${1:?usage: $0 PROTOCOL PREFLIGHT_RECEIPT EVALUATOR_REPO CACHE_ROOT}"
preflight="${2:?missing preflight receipt}"
evaluator_repo="${3:?missing evaluator repository}"
cache_root="${4:?missing evaluator cache root}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo="$(cd "${script_dir}/../.." && pwd)"
python="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["assets"]["runtime_manifest"]["path"])' "${protocol}")"
python="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["environment_prefix"]+"/bin/python")' "${python}")"
...
"${python}" "${script_dir}/experiment.py" launch \
  --protocol "${protocol}" --preflight-receipt "${preflight}"
