...
write_json(W/'receipts/scheduler_stalled_lanes_resume.json',dict(status='AUTHORIZED_TECHNICAL_RESUME',reason='Supplement untouched stalled lanes 0,2,5 after non-atomic completion marker delivery; completed formal jobs are preserved. Input directories remain exclusive.',completed_jobs_preserved=8,not_run_jobs_to_resume=50,scientific_jobs_repeated=0,resume_script_sha256=sha256(Path(__file__)),original_execution_freeze_sha256=sha256(W/'receipts/execution_code_freeze.json'),resumed_epoch=time.time()))
def worker(gpu):
 results=[]
 for job in planned[gpu::8]:
  ...
  if rp.exists():
   ...
   while True:
    try:
     if ready.exists() and load(ready)['status']=='PASS':break
...
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
 futures=[pool.submit(worker,g) for g in (0,2,5)]; results=[x for f in futures for x in f.result()]
print('STALLED_LANES_COMPLETE',len(results),flush=True)
