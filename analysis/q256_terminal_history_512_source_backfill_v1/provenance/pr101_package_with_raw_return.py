...
def main():
    ...
    require(load(work/'receipts/ect_raw_backup_complete.json')['status']=='PASS','Raw artifact return must complete before instance release')
    ...
    write_json(payload/'ARCHIVE_CONTENTS.json',dict(experiment_id=EXPERIMENT_ID,protocol_sha256=PROTOCOL_SHA,
       ...
       large_generated_artifacts='Preserved and hash-verified on ECT002 under /data/raw/ECT/q256_terminal_history_512_source_backfill_v1/raw_evaluation_artifacts; compact tar includes their immutable hashes and receipts',
    ...
    write_json(work/'PACKAGE_RECEIPT.json',dict(status='PASS',experiment_id=EXPERIMENT_ID,
        ...
        return_directory='/data/raw/ECT/'+EXPERIMENT_ID,raw_artifact_backup_directory='/data/raw/ECT/'+EXPERIMENT_ID+'/raw_evaluation_artifacts',raw_backup_receipt_sha256=sha256(work/'receipts/ect_raw_backup_complete.json'),packaging_script_sha256=sha256(Path(__file__)),remote_sha_verified=False))
