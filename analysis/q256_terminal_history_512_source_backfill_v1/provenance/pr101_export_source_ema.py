...
def export(item):
 ...
 out=ROOT/'source_snapshots'/('job%03d'%i); out.mkdir(parents=True,exist_ok=False)
 s=torch.load(r['checkpoint_path'],map_location='cpu',weights_only=False)
 ...
 rec=dict(status='PASS',seed=r['seed'],arm=r['arm'],source_state_path=r['checkpoint_path'],source_state_sha256=r['checkpoint_sha256'],ema_canonical_sha256=h,snapshot_sha256=sha(p),cur_nimg=512000,attempted_iteration=4000,source_hash_verification='Previously completed full source inventory; not repeated during export',exported_on='ECT002',exported_epoch=time.time(),exporter_sha256=sha(Path(__file__)))
...
if __name__=='__main__':
 ...
 with concurrent.futures.ProcessPoolExecutor(max_workers=4) as pool: records=list(pool.map(export,enumerate(rows)))
 (ROOT/'receipts/source_export_complete.json').write_text(json.dumps(dict(status='PASS',count=len(records),records=records),indent=2)+'\n')
