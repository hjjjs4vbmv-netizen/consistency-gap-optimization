...
def sha(p):
 ...
 return h.hexdigest()
def worker(g):
 sock=R/('a100-lane%d.sock'%(24+g)); ssh=['ssh','-o','BatchMode=yes','-S',str(sock),'-p','27815','root@px-cloud1.matpool.com']
 for i in range(g,58,8):
  ...
  start=time.time()
  with (R/'logs'/('raw_return_job%03d.log'%i)).open('xb') as log:subprocess.run(cmd,stdout=log,stderr=subprocess.STDOUT,check=True)
  ...
  if i<8:assert sha(exdir/'network-snapshot.pkl')==ex['snapshot_sha256']
   ...
   import os
   os.link(source,exdir/'network-snapshot.pkl')
