...
def main(argv=None):
    ...
    net, optimizer, scaler_state, loss_fn_state, meta = load_training_state(
        a.training_state, device, lr=1e-4, betas=(0.9, 0.999), eps_opt=1e-8)
    ...
    # candidate arm: deepcopy net + optimizer from the same state (identical
    # params, moments, step count; only the schedule/gap differs).
    import copy
    net_ref = net
    net_cand = copy.deepcopy(net)
    opt_ref = optimizer
    opt_cand = torch.optim.RAdam(net_cand.parameters(), lr=1e-4, betas=(0.9, 0.999), eps=1e-8)
