#!/usr/bin/env python3
"""Independently verify a checkpoint archive and its sender manifest.

This verifier intentionally has no repository-specific imports or protocol
rules. A receiver can copy it with any sender manifest to an independent
checkout and verify the archive bytes plus every declared checkpoint digest
before emitting a portable receipt.
"""
...
def load_object(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise SystemExit(f"[verify_handoff] ERROR: cannot read {label}: {exc}") from exc
    if not isinstance(value, dict):
        raise SystemExit(f"[verify_handoff] ERROR: {label} must be a JSON object")
    return value
    ...
    with tarfile.open(archive, "r:*") as handle:
        ...
        for member in handle.getmembers():
            ...
            if target != base and base not in target.parents:
                raise SystemExit("[verify_handoff] ERROR: archive contains an unsafe path")
...
def verify(sender_manifest: Path, archive: Path, verifier_identity: str) -> dict[str, Any]:
    ...
    if not isinstance(checkpoints, list) or not checkpoints:
        raise SystemExit("[verify_handoff] ERROR: sender manifest lacks checkpoints")
    if not verifier_identity.strip():
        raise SystemExit("[verify_handoff] ERROR: --verifier-identity must be non-empty")
    ...
    with tempfile.TemporaryDirectory(prefix="handoff-verify-") as temp:
        ...
        for item in checkpoints:
            if not isinstance(item, dict):
                raise SystemExit("[verify_handoff] ERROR: malformed checkpoint declaration")
            checkpoint_id = item.get("checkpoint_id")
            filename = item.get("filename")
            expected_sha = item.get("sha256")
            if not all(isinstance(value, str) and value for value in (checkpoint_id, filename, expected_sha)):
                raise SystemExit("[verify_handoff] ERROR: incomplete checkpoint declaration")
    ...
    return {
        "schema_version": 1,
        "verification_role": "independent receiver",
        ...
        "claim_effect": (
            ...
            "Checkpoint identity verification failed; this handoff must not support its declared evidence."
        ),
    }
...
def main() -> None:
    ...
    if receipt["status"] != "passed":
        raise SystemExit("[verify_handoff] ERROR: verification failed")
