...
for report in preflight_report smoke_report parity_report; do
  python_bin_check="${python_bin}"
  "${python_bin_check}" -c 'import json,sys; assert json.load(open(sys.argv[1]))["status"]=="PASS"' "${analysis}/${report}.json"
done
[[ "$(python -c 'import json,sys; print(json.load(open(sys.argv[1]))["job_count"])' "${analysis}/evaluation_manifest.json")" == 272 ]] || { echo "evaluation manifest missing" >&2; exit 2; }
...
"${python_bin}" -c 'import json,subprocess,sys; out=sys.argv[1]; seeds=range(201,209); pids=list(map(int,sys.argv[2].split(","))); rows=subprocess.check_output(["nvidia-smi","--query-gpu=index,uuid,name,driver_version","--format=csv,noheader"],text=True).splitlines(); payload={"schema":"ect.q128-fresh-launch-receipt/v1","status":"LAUNCHED","branch":sys.argv[3],"freeze_commit":sys.argv[4],"freeze_commit_timestamp":sys.argv[5],"formal_launch_utc":sys.argv[6],"freeze_precedes_formal_logs":sys.argv[5]<sys.argv[6],"jobs":[{"seed":s,"gpu_index":s-201,"pid":pids[s-201],"gpu_identity":rows[s-201],"run_dir":f"{sys.argv[7]}/formal/seed{s}","log":f"{sys.argv[7]}/formal_logs/seed{s}.log"} for s in seeds]}; open(out,"w").write(json.dumps(payload,indent=2,sort_keys=True)+"\n")' \
  "${analysis}/launch_receipt.json" "$(IFS=,; echo "${pids[*]}")" "${branch}" "${freeze_sha}" "${freeze_time}" "${started}" "${run_root}"
