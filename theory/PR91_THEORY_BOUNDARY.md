...
- IA has lower FID and KID in all 30 repeated seed-by-checkpoint-by-NFE cells
  through 6,400 kimg. The cells comprise five checkpoints and two NFE settings
  within each of three independent training seeds.
...
- Three-seed means after 8,960 kimg are highly sensitive to seed 103 and cannot
  serve as a common late-training ranking across the three trajectories.

The first item is a repeated-measurement pattern across three training seeds,
not 30 independent replications. The second is evidence for trajectory
instability in one paired seed. They are distinct empirical statements.
...
| IA has lower FID and KID throughout the evaluated horizon through 6,400 kimg in each of the three paired seeds. | Supported in the evaluated matrix | All 30 repeated seed-by-checkpoint-by-NFE cells favor IA for each metric; the independent training unit is the seed. |
...
| A late three-seed endpoint mean identifies a stable schedule ranking. | Not supported | The mean is highly sensitive to the unstable seed103 trajectory. |

## 4. Main-text wording licensed by the evidence

> Through 6,400 kimg, IA has lower FID and KID in every evaluated checkpoint
> and NFE cell within each of the three paired ImageNet-64 training seeds. Late
> training exposes a separate stability phenomenon: seed 103 degrades in both
> arms, making the three-seed endpoint mean highly sensitive to that trajectory.
