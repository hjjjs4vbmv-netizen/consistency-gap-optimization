...
- Version Label: q128_fresh_regime_history_n8_v1_results
...
There is a second independent governance failure: seed999 smoke stopped before training with `ModuleNotFoundError: click`, and exact-resume/switch parity was not run. Formal training began only after the experiment owner explicitly activated `owner_directed_operational_waiver_003`, which recorded both gates as `WAIVED_NOT_PASSED`. Either this waiver or the late-AMP violation is sufficient to prevent a clean confirmatory claim; neither changes the frozen numerical analysis.
...
The decode gate verified 272 unique opaque IDs, all SEALED_PASS receipts, effective-cohort membership, evaluator/source/dataset hashes, category coverage, KID/FID shared-feature identity, and the hashes of all sealed scalar artifacts. The PR contains the per-seed estimands, statistical summaries, verdicts, and governance evidence. The complete 272-cell decoded input is deliberately excluded from GitHub and remains in the SHA-verified persistent archive at `/mnt/q128_fresh_regime_history_n8_v1_archive`.
