...
\subsection{诊断规划的边界(P1 附庸性 + Arm C + 未来预测)}
...
因此:
\begin{itemize}
  \item \textbf{不声称}残差是质量诊断信号(被 P1 + 未来预测证伪)
  \item \textbf{不声称}残差有害或可修正(被 $g{=}1.3$ 赢 FID + Arm C 证伪)
  \item \textbf{不投} ICLR 主会(诊断信号论题不成立)
\end{itemize}
\textbf{唯一剩余活口:}优化器级未来预测(早期 $R_{\mathrm{opt}}$,需重训存早期
optimizer state),但鉴于 Arm C 翻盘概率不高。Go/No-Go 交 leader 判断。
